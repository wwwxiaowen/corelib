...
static const NSInteger kDefaultCacheMaxCacheAge = 60 * 60 * 24 * 7 * 48; // 1 week
...
- (id)init
{
    return [self initWithNamespace:@"huivoswift"];
}
...
- (id)initWithNamespace:(NSString *)ns
{
    if ((self = [super init]))
    {
        NSString *fullNamespace = [@"com.huivo.SDWebImageCache." stringByAppendingString:ns];

        // Create IO serial queue
        _ioQueue = dispatch_queue_create("com.huivo.SDWebImageCache", DISPATCH_QUEUE_SERIAL);
